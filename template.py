...
# Data-Preprocessing
data = pd.read_csv('./Datasets/housing.csv')     # Add path to the dataset here
X = data.iloc[:, :-1]
y = data.iloc[:, -1]
...
poly = PolynomialFeatures(degree = 2)                       # You might want to try with different values of degree
...
predictions_svr= np.reshape(predictions_svr, (147, 1))              # __++__ This raises error (Ex. cannot convert 20000 into 147), as I have done this for specific dataset. To resolve just give (<num>, 1) where <num> is shown in the error which is raised.
...
predictions_dr= np.reshape(predictions_dr, (147, 1))                # Similary to __++__
...
predictions_rfr = np.reshape(predictions_rfr, (147, 1))             # Similary to __++__

# Calculating the Result in terms of errors
from sklearn import metrics
result = list()                                                                     # To remove some regression algorithms, don't forget to make the changes here as well.
